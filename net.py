...
class Net(object):
    ...
    def discriminator(self, data_313, reuse=False):
        ...
        with tf.variable_scope('D', reuse=reuse):
            if self.easy:
                ...
                # 22x22
                conv_num += 1
                conv_2 = conv2d('d_conv_{}'.format(conv_num), conv_1, [4, 4, 64, 32], stride=1, wd=None)

                # 11x11
                conv_num += 1
                conv_3 = conv2d('d_conv_{}'.format(conv_num), conv_2, [4, 4, 32, 1], stride=1, relu=False, wd=None, sigmoid=True)
    ...
    def discriminator_loss(self, original, colorized):
        ...
        real_score = tf.reduce_sum(original) * downscale / self.batch_size
