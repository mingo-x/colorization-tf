...
def Linear(scope, input, dim, bias_initializer=tf.constant_initializer(0.0), kernel_initializer=None):
...
def subconv2d(scope, input, kernel_size, relu=True, wd=nilboy_weight_decay):
    with tf.variable_scope(scope) as scope:
        kernel = variable_with_weight_decay('weights', 
                                            shape=kernel_size,
                                            stddev=5e-2,
