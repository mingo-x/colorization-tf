...
def JBU(ab_ss, l, k=3, scale=4):
    ...
    a = cv2.ximgproc.jointBilateralFilter(l, ab_ic[:, :, 0: 1], d=-1, sigmaColor=15., sigmaSpace=3.)
    b = cv2.ximgproc.jointBilateralFilter(l, ab_ic[:, :, 1:], d=-1, sigmaColor=15., sigmaSpace=3.)
    ab = np.dstack((a, b))
    # h, w, c = ab_ss.shape
    # h *= scale
    # w *= scale
